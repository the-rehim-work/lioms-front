...
import { useCompanies, useCreateCompany, useUpdateCompany, useDeleteCompany } from "@/hooks/use-lookups";
import { useAuthStore } from "@/stores/auth";
import type { CompanyGetDTO, CompanyPostDTO, CompanyPutDTO } from "@/types";

const empty = { name: "", alias: "" };
...
export default function Companies() {
  ...
  const handleCreate = () => {
    ...
    const dto: CompanyPostDTO = { ...form };
    create.mutate(dto, { onSuccess: () => setModalOpen(false) });
  };

  const handleInlineEdit = (row: CompanyGetDTO, changes: Record<string, unknown>) => {
    const dto: CompanyPutDTO = {
      id: row.id,
      name: (changes.name as string) ?? row.name,
      alias: (changes.alias as string) ?? row.alias,
    };
    update.mutate({ id: row.id, dto });
  };

  const columns: Column<CompanyGetDTO>[] = [
    { key: "id", header: "ID", className: "w-16" },
    { key: "name", header: "Ad", editable: isAdmin() },
    { key: "alias", header: "Qısaltma", editable: isAdmin() },
  ];

  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">Qurumlar</h1>
        {isAdmin() && (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Yeni qurum">
        <div className="space-y-4">
          <Field label="Ad" value={form.name} onChange={(e) => setForm({ ...form, name: (e.target as HTMLInputElement).value })} />
          <Field label="Qısaltma" value={form.alias} onChange={(e) => setForm({ ...form, alias: (e.target as HTMLInputElement).value })} />
